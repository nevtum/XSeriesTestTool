import unittest
from datablockmodels import *

class TestSDBHandle(unittest.TestCase):
    def setUp(self):
        f = open('sdbmockdata.txt', 'r')
        data = f.readline().split()
        self.sdb1 = sdbMdl()
        self.sdb1.setdata(data)
        self.sdb1.readmetadata('packets.xml')
        data = f.readline().split()
        self.sdb2 = sdbMdl()
        self.sdb2.setdata(data)
        self.sdb2.readmetadata('packets.xml')
        
    def testpacketinfo(self):
        self.assertEquals('SDB', self.sdb1.packetinfo())

    def test_version(self):
        self.assertEquals('0101', self.sdb1.getitem('version'))
        self.assertEquals('0102', self.sdb2.getitem('version'))

    def test_GMID(self):
        self.assertEquals('123456', self.sdb1.getitem('gmid'))
        self.assertEquals('563412', self.sdb2.getitem('gmid'))
        
    def test_status_byte_1(self):
        sdb = self.sdb1
        self.assertEquals(True ,sdb.getitem('idle'))
        self.assertEquals(False, sdb.getitem('gamecycle'))
        self.assertEquals(True, sdb.getitem('poweredup'))
        self.assertEquals(False, sdb.getitem('reset'))
        self.assertEquals(True, sdb.getitem('cccetransfercomplete'))
        self.assertEquals(False, self.sdb2.getitem('idle'))
        self.assertEquals(True, self.sdb2.getitem('gamecycle'))
        self.assertEquals(False, self.sdb2.getitem('poweredup'))
        self.assertEquals(True, self.sdb2.getitem('reset'))
        self.assertEquals(False, self.sdb2.getitem('cccetransfercomplete'))
        
    def test_status_byte_2(self):
        self.assertEquals(True, self.sdb1.getitem('largewin'))
        self.assertEquals(False, self.sdb1.getitem('collectcash'))
        self.assertEquals(True, self.sdb1.getitem('cancelcredit'))
        self.assertEquals(False, self.sdb1.getitem('progressivewin'))
        self.assertEquals(True, self.sdb1.getitem('manufacturerspecificwin1'))
        self.assertEquals(False, self.sdb1.getitem('manufacturerspecificwin2'))
        self.assertEquals(True, self.sdb1.getitem('manufacturerspecificwin3'))
        self.assertEquals(False, self.sdb2.getitem('largewin'))
        self.assertEquals(True, self.sdb2.getitem('collectcash'))
        self.assertEquals(False, self.sdb2.getitem('cancelcredit'))
        self.assertEquals(True, self.sdb2.getitem('progressivewin'))
        self.assertEquals(False, self.sdb2.getitem('manufacturerspecificwin1'))
        self.assertEquals(True, self.sdb2.getitem('manufacturerspecificwin2'))
        self.assertEquals(False, self.sdb2.getitem('manufacturerspecificwin3'))
        
    def test_status_byte_3(self):
        self.assertEquals(True, self.sdb1.getitem('maindooropen'))
        self.assertEquals(False, self.sdb1.getitem('securitycageopen'))
        self.assertEquals(True, self.sdb1.getitem('displayerror'))
        self.assertEquals(False, self.sdb1.getitem('selfauditerror'))
        self.assertEquals(True, self.sdb1.getitem('memoryerror'))
        self.assertEquals(False, self.sdb1.getitem('cashinputerror'))
        self.assertEquals(True, self.sdb1.getitem('cashoutputerror'))
        self.assertEquals(False, self.sdb2.getitem('maindooropen'))
        self.assertEquals(True, self.sdb2.getitem('securitycageopen'))
        self.assertEquals(False, self.sdb2.getitem('displayerror'))
        self.assertEquals(True, self.sdb2.getitem('selfauditerror'))
        self.assertEquals(False, self.sdb2.getitem('memoryerror'))
        self.assertEquals(True, self.sdb2.getitem('cashinputerror'))
        self.assertEquals(False, self.sdb2.getitem('cashoutputerror'))
        
    def test_status_byte_4(self):
        self.assertEquals(True, self.sdb1.getitem('auditmode'))
        self.assertEquals(False, self.sdb1.getitem('testmode'))
        self.assertEquals(True, self.sdb1.getitem('powersave'))
        self.assertEquals(False, self.sdb1.getitem('subsidiaryequipmentplaysuspended'))
        self.assertEquals(True, self.sdb1.getitem('mechanicalmeterdisconnected'))
        self.assertEquals(False, self.sdb1.getitem('manufacturerspecificerror1'))
        self.assertEquals(True, self.sdb1.getitem('manufacturerspecificerror2'))
        self.assertEquals(False, self.sdb2.getitem('auditmode'))
        self.assertEquals(True, self.sdb2.getitem('testmode'))
        self.assertEquals(False, self.sdb2.getitem('powersave'))
        self.assertEquals(True, self.sdb2.getitem('subsidiaryequipmentplaysuspended'))
        self.assertEquals(False, self.sdb2.getitem('mechanicalmeterdisconnected'))
        self.assertEquals(True, self.sdb2.getitem('manufacturerspecificerror1'))
        self.assertEquals(False, self.sdb2.getitem('manufacturerspecificerror2'))
        
    def test_status_byte_5(self):
        self.assertEquals(True, self.sdb1.getitem('cancelcrediterror'))
        self.assertEquals(False, self.sdb2.getitem('cancelcrediterror'))
        
    def test_multi_game(self):
        pass
    
    def test_meters(self):
        self.assertEquals('$127.05', self.sdb1.getitem('turnover'))
        self.assertEquals('$12345678.90', self.sdb1.getitem('totalwins'))
        self.assertEquals('$2025.40', self.sdb1.getitem('cashbox'))
        self.assertEquals('$12345678.90', self.sdb1.getitem('cancelcredits'))
        self.assertEquals('99999999', self.sdb1.getitem('gamesplayed'))
        self.assertEquals('$561.55', self.sdb1.getitem('moneyin'))
        self.assertEquals('$22222222.22', self.sdb1.getitem('moneyout'))
        self.assertEquals('$12345678.90', self.sdb1.getitem('cashin'))
        self.assertEquals('$6040.20', self.sdb1.getitem('cashout'))
        self.assertEquals('1111111111', self.sdb1.getitem('credits'))
        self.assertEquals('$0.00', self.sdb1.getitem('miscellaneousaccrual'))
        self.assertEquals('55555555', self.sdb1.getitem('totalpowerups'))
        self.assertEquals('00005684', self.sdb1.getitem('gamesplayedsincelastpowerup'))
        self.assertEquals('77777777', self.sdb1.getitem('gamesplayedsincelastdooropen'))
        self.assertEquals('$0.01', self.sdb1.getitem('basecreditvalue'))
        self.assertEquals('$12345678.90', self.sdb2.getitem('turnover'))
        self.assertEquals('$27.24', self.sdb2.getitem('totalwins'))
        self.assertEquals('$12345678.90', self.sdb2.getitem('cashbox'))
        self.assertEquals('$785.22', self.sdb2.getitem('cancelcredits'))
        self.assertEquals('88888888', self.sdb2.getitem('gamesplayed'))
        self.assertEquals('$11111111.11', self.sdb2.getitem('moneyin'))
        self.assertEquals('$13.43', self.sdb2.getitem('moneyout'))
        self.assertEquals('$2036.40', self.sdb2.getitem('cashin'))
        self.assertEquals('$33333333.33', self.sdb2.getitem('cashout'))
        self.assertEquals('0000256382', self.sdb2.getitem('credits'))
        self.assertEquals('$0.00', self.sdb2.getitem('miscellaneousaccrual'))
        self.assertEquals('00000174', self.sdb2.getitem('totalpowerups'))
        self.assertEquals('66666666', self.sdb2.getitem('gamesplayedsincelastpowerup'))
        self.assertEquals('00004321', self.sdb2.getitem('gamesplayedsincelastdooropen'))
        self.assertEquals('$52.63', self.sdb2.getitem('basecreditvalue'))
        
    def test_port_status(self):
        self.assertEquals(True, self.sdb1.getitem('sefport1'))
        self.assertEquals(True, self.sdb1.getitem('sefport2'))
        self.assertEquals(True, self.sdb1.getitem('sefport3'))
        self.assertEquals(True, self.sdb1.getitem('sefport4'))
        self.assertEquals(True, self.sdb1.getitem('sefport5'))
        self.assertEquals(True, self.sdb1.getitem('sefport6'))
        self.assertEquals(False, self.sdb2.getitem('sefport1'))
        self.assertEquals(False, self.sdb2.getitem('sefport2'))
        self.assertEquals(False, self.sdb2.getitem('sefport3'))
        self.assertEquals(False, self.sdb2.getitem('sefport4'))
        self.assertEquals(False, self.sdb2.getitem('sefport5'))
        self.assertEquals(False, self.sdb2.getitem('sefport6'))
        
    def test_ascii_text(self):
        self.assertEquals('GM001700', self.sdb1.getitem('programid1'))
        self.assertEquals('', self.sdb1.getitem('programid2'))
        self.assertEquals('SDM00304', self.sdb1.getitem('programid3'))
        self.assertEquals('S2X00706', self.sdb1.getitem('programid4'))
        self.assertEquals('GM001802', self.sdb2.getitem('programid1'))
        self.assertEquals('ABCD1234', self.sdb2.getitem('programid2'))
        self.assertEquals('SDM12345', self.sdb2.getitem('programid3'))
        self.assertEquals('S2X12345', self.sdb2.getitem('programid4'))
        
    def test_rtp(self):
        self.assertEquals('99.99%', self.sdb1.getitem('prtp'))
        self.assertEquals('93.96%', self.sdb2.getitem('prtp'))
        
    def test_secondary_functions(self):
        self.assertEquals(True, self.sdb1.getitem('stdlinkedprogressivesupport'))
        self.assertEquals(False, self.sdb2.getitem('stdlinkedprogressivesupport'))
        
class TestMDBHandle(unittest.TestCase):
    def setUp(self):
        f = open('mdbmockdata.txt', 'r')
        data = f.readline().split()
        self.mdb1 = mdbMdl()
        self.mdb1.setdata(data)
        data = f.readline().split()
        self.mdb2 = mdbMdl()
        self.mdb2.setdata(data)

    def testmdbstuff(self):
        self.assertEquals('123456', self.mdb1.GMID())
        self.assertEquals('887766', self.mdb2.GMID())
        self.assertEquals('5AA5', self.mdb1.versionNr())
        self.assertEquals('ABCD', self.mdb2.versionNr())
        self.assertEquals('1.05', self.mdb1.mdbType())
        self.assertEquals('1.92', self.mdb2.mdbType())
        
    def testBillAcceptorStatusByte1(self):
        self.assertEquals(False, self.mdb1.stackerDoorOpen())
        self.assertEquals(False, self.mdb1.stackerCommsError())
        self.assertEquals(False, self.mdb1.stackerFailure())
        self.assertEquals(False, self.mdb1.stackerFull())
        self.assertEquals(False, self.mdb1.stackerRemoved())
        self.assertEquals(False, self.mdb1.stackerOutOfService())
        self.assertEquals(True, self.mdb2.stackerDoorOpen())
        self.assertEquals(True, self.mdb2.stackerCommsError())
        self.assertEquals(True, self.mdb2.stackerFailure())
        self.assertEquals(True, self.mdb2.stackerFull())
        self.assertEquals(True, self.mdb2.stackerRemoved())
        self.assertEquals(True, self.mdb2.stackerOutOfService())
        
    def testMiscStatusByte1(self):
        self.assertEquals(True, self.mdb1.cashBoxDropDoorOpen())
        self.assertEquals(True, self.mdb1.printerPaperLow())
        self.assertEquals(True, self.mdb1.validTicketOut())
        self.assertEquals(True, self.mdb1.printerFault())
        self.assertEquals(True, self.mdb1.paperEmpty())
        self.assertEquals(True, self.mdb1.validTicketIn())
        self.assertEquals(False, self.mdb2.cashBoxDropDoorOpen())
        self.assertEquals(False, self.mdb2.printerPaperLow())
        self.assertEquals(False, self.mdb2.validTicketOut())
        self.assertEquals(False, self.mdb2.printerFault())
        self.assertEquals(False, self.mdb2.paperEmpty())
        self.assertEquals(False, self.mdb2.validTicketIn())
        
    def testMiscStatusByte2(self):
        self.assertEquals(False, self.mdb1.ticketInCommsError())
        self.assertEquals(False, self.mdb1.ticketInRejectedByHost())
        self.assertEquals(False, self.mdb1.tenRejects())
        self.assertEquals(False, self.mdb1.miscTicketInError())
        self.assertEquals(False, self.mdb1.ticketLowerThanBCV())
        self.assertEquals(False, self.mdb1.ticketStackingDone())
        self.assertEquals(True, self.mdb2.ticketInCommsError())
        self.assertEquals(True, self.mdb2.ticketInRejectedByHost())
        self.assertEquals(True, self.mdb2.tenRejects())
        self.assertEquals(True, self.mdb2.miscTicketInError())
        self.assertEquals(True, self.mdb2.ticketLowerThanBCV())
        self.assertEquals(True, self.mdb2.ticketStackingDone())
        
    def testNotesMeters(self):
        self.assertEquals('5544332211', self.mdb1.nr5DollarNotes())
        self.assertEquals('1122334455', self.mdb2.nr5DollarNotes())
        self.assertEquals('1122222233', self.mdb1.nr10DollarNotes())
        self.assertEquals('3322222211', self.mdb2.nr10DollarNotes())
        self.assertEquals('4455555566', self.mdb1.nr20DollarNotes())
        self.assertEquals('6655555544', self.mdb2.nr20DollarNotes())
        self.assertEquals('1122222233', self.mdb1.nr50DollarNotes())
        self.assertEquals('3322222211', self.mdb2.nr50DollarNotes())
        self.assertEquals('4455555566', self.mdb1.nr100DollarNotes())
        self.assertEquals('6655555544', self.mdb2.nr100DollarNotes())
        self.assertEquals('1122222233', self.mdb1.ticketsAccepted())
        self.assertEquals('3322222211', self.mdb2.ticketsAccepted())
        self.assertEquals('4455555566', self.mdb1.ticketsRejected())
        self.assertEquals('6655555544', self.mdb2.ticketsRejected())
        self.assertEquals('1122222233', self.mdb1.totBillsAcceptedSpare())
        self.assertEquals('3322222211', self.mdb2.totBillsAcceptedSpare())
        self.assertEquals('$44555555.66', self.mdb1.valBillsAccepted())
        self.assertEquals('$66555555.44', self.mdb2.valBillsAccepted())
        self.assertEquals('112222222233', self.mdb1.totBillsAccepted())
        self.assertEquals('332222222211', self.mdb2.totBillsAccepted())
        self.assertEquals('25111997', self.mdb1.dateTicketPrinted())
        self.assertEquals('14032011', self.mdb2.dateTicketPrinted())
        self.assertEquals('230137', self.mdb1.timeTicketPrinted())
        self.assertEquals('142249', self.mdb2.timeTicketPrinted())
        self.assertEquals('11222222222222222233', self.mdb1.printerID())
        self.assertEquals('33222222222222222211', self.mdb2.printerID())
        self.assertEquals('4455555566', self.mdb1.ticketAmount())
        self.assertEquals('6655555544', self.mdb2.ticketAmount())
        self.assertEquals('7788888899', self.mdb1.sequentialNr())
        self.assertEquals('9988888877', self.mdb2.sequentialNr())
        
    def test_machine_config_bytes(self):
        self.assertEquals(True, self.mdb1.hopperConfigured())
        self.assertEquals(True, self.mdb1.BNAConfigured())
        self.assertEquals(True, self.mdb1.printerConfigured())
        self.assertEquals(False, self.mdb2.hopperConfigured())
        self.assertEquals(False, self.mdb2.BNAConfigured())
        self.assertEquals(False, self.mdb2.printerConfigured())
        self.assertEquals(False, self.mdb1.stdProgPayDone())
        self.assertEquals(False, self.mdb1.mystProgPayDone())
        self.assertEquals(False, self.mdb1.ccceTxComplete())
        self.assertEquals(False, self.mdb1.mystProgWin())
        self.assertEquals(True, self.mdb2.stdProgPayDone())
        self.assertEquals(True, self.mdb2.mystProgPayDone())
        self.assertEquals(True, self.mdb2.ccceTxComplete())
        self.assertEquals(True, self.mdb2.mystProgWin())
        self.assertEquals(True, self.mdb1.stdProgWinNotification())
        self.assertEquals(True, self.mdb1.mystProgWinNotification())
        self.assertEquals(True, self.mdb1.ccceIncDec())
        self.assertEquals(True, self.mdb1.stdProgPayment())
        self.assertEquals(True, self.mdb1.mystProgPayment())
        self.assertEquals(True, self.mdb1.stdProgPoolVal())
        self.assertEquals(True, self.mdb1.mystProgPoolVal())
        self.assertEquals(False, self.mdb2.stdProgWinNotification())
        self.assertEquals(False, self.mdb2.mystProgWinNotification())
        self.assertEquals(False, self.mdb2.ccceIncDec())
        self.assertEquals(False, self.mdb2.stdProgPayment())
        self.assertEquals(False, self.mdb2.mystProgPayment())
        self.assertEquals(False, self.mdb2.stdProgPoolVal())
        self.assertEquals(False, self.mdb2.mystProgPoolVal())
        self.assertEquals(False, self.mdb1.ticketInPort1Set())
        self.assertEquals(False, self.mdb1.timeBroadcastSet())
        self.assertEquals(True, self.mdb2.ticketInPort1Set())
        self.assertEquals(True, self.mdb2.timeBroadcastSet())
        
    def test_tito_meters(self):
        self.assertEquals('$1010101.01', self.mdb1.amtStdProgWin())
        self.assertEquals('$20202020.20', self.mdb2.amtStdProgWin())
        self.assertEquals('$44555555.66', self.mdb1.amtMystProgWin())
        self.assertEquals('$66555555.44', self.mdb2.amtMystProgWin())
        self.assertEquals('$561.52', self.mdb1.amtCCCETxIn())
        self.assertEquals('$10101010.10', self.mdb2.amtCCCETxIn())
        self.assertEquals('$22333333.44', self.mdb1.amtTicketOut())
        self.assertEquals('$44333333.22', self.mdb2.amtTicketOut())
        self.assertEquals('$30303030.30', self.mdb1.amtTicketIn())
        self.assertEquals('$1960.40', self.mdb2.amtTicketIn())

if __name__ == '__main__':
    unittest.main()